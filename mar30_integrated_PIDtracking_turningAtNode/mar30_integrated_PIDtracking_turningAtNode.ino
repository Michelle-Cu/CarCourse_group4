...
void loop() {
  ...
  val[1] = digitalRead(analog1); 
  val[2] = digitalRead(analog2); 
  val[3] = digitalRead(analog3); 
  val[4] = digitalRead(analog4); 
  val[5] = digitalRead(analog5);
  // for(int i=1; i<=5; i++){
  ...
  // // On the line: isTurning = 0
  // // Reaches node: isTurning = 1
  // // Sensor senses all white and starts spinning: isTurning = 2
  // // Sensor senses a line, triggering the 90 degree turn: isTurning = 3
  ...
  // }
  // if (isTurning == 1){
  //   if(count == 0) isTurning = 2;
  //   // else if(count <= 3 && (val[2] == 80 || val[3] == 80 || val[4] == 80)) isTurning = 3;
  // }
  ...
  // // else if(isTurning == 3 && count <= 3 && (val[1] == 80 && val[5] != 80)){
  // //   isTurning = 0;
  // //   delay(10);
  // // }
  // else if(isTurning == 2 || isTurning == 3) turnRight(60, 60);
  // else Tracking();
  ...
  // --- 2. NODE DETECTION ---
  if (count == 5 && isTurning == 0) {
    // Inch forward so the wheels are centered on the square
    MotorWriting(Tp, Tp); 
    delay(150); 
    isTurning = 1; 
    t1Millis = millis();
  ...
  // if (isTurning == 1) {
  //   // Wait for the sensors to clear the node or time out
  //   if (count == 0 || (millis() - tMillis > 200)) {
  //     isTurning = 2; 
  //   }
  // }

    if (isTurning == 1 && count <= 1 && millis() - t1Millis > 1200  ) {
      isTurning = 2;
      t2Millis = millis();
    }
  ...
  if (isTurning == 2) {
    // Search for the line
    if (count <= 3 && (val[2] > 0 || val[3] > 0 || val[4] > 0 ) && millis() - t2Millis > 500) {
      isTurning = 0;
      tp++; // Move to the next instruction for the next node
      // stopMotors();
      // delay(100);
    }
    else {
      // Execute instruction from our list
      if (t[tp] == 2) turnLeft(60, 60);       // Turn Left
      else if (t[tp] == 4) turnRight(70, 70); // Turn Right
      else if (t[tp] == 3) turnRight(80, 80); // U-Turn (Spin faster)
    }
  } 
  else {
