            ...
            for msg in messages:
                ...
                if msg.startswith("S:"):
                    try:
                        parts = msg[2:].split(",")
                        arduino_done = int(parts[0])
                        arduino_moves = [int(x) for x in parts[1:]]
                        ...
                        if diff > 0:
                            for _ in range(diff):
                                if action_queue:
                                    popped = action_queue.pop(0)
                                    log.info(f"🚗 Move done. Popped {Action(popped).name}. Remaining: {len(action_queue)}")
                            moves_done_python = arduino_done
                        ...
                        if needs_resend or arduino_count < 3:
                            if action_queue:
                                ...
                                batch_str = ",".join(map(str, [int(a) for a in moves_to_send]))
                                bridge.send(f"nxtMove:{start_idx},{batch_str}")
                                log.info(f"Sent Batch: nxtMove:{start_idx},{batch_str}")
                        ...
                        log.error(f"⚠️ Malformed Status: {msg} ({e})")

                elif msg == "reqFirstMove":
                    log.info("Arduino restarted — resending from beginning")
                    action_queue = list(original_action_queue)
                    moves_done_python = 0
                    moves_to_send = action_queue[:3]
                    batch_str = ",".join(map(str, [int(a) for a in moves_to_send]))
                    bridge.send(f"nxtMove:1,{batch_str}")
