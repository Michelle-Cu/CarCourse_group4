...
class HM10ESP32Bridge:
        ...
        while (time.time() - start_time) < 10.0:
            for entry in self._read_bt_com_payloads():
                if "OK+RESET" in entry:
                    time.sleep(6) # Wait for ESP32 to reboot
                    return True
            time.sleep(0.01)
        return False
    ...
    def listen(self):
        ...
        # Returns a LIST to prevent character-by-character printing
        return [l for l in logs if not (l.startswith("OK+") or l.startswith("ERROR+"))]

    def send(self, text):
        """Sends data with newline and stability delay."""
        if not text.endswith("\n"):
            text += "\n"
        self.ser.write(text.encode('utf-8'))
        time.sleep(0.02) # Stability delay to prevent buffer overflow
